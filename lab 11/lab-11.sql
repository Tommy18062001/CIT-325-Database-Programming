...
-- Insert an example row into the logger table
DECLARE
    CURSOR get_row IS
        SELECT *
        FROM item 
...
-- Create an item_trig trigger that performs tasks for INSERT and UPDATE statements
...
-- Create an item_delete_trig trigger that performs tasks for a DELETE statement:
...
-- Alter the fk_item_1 foreign key constraint from the item. 
ALTER TABLE item

-- Drop the fk_item_1 foreign key constraint from the item table.
...
OL item_id        FORMAT 9999 HEADING "Item|ID #"
...
--Update the Star Wars to test the updating event trigger
UPDATE item
