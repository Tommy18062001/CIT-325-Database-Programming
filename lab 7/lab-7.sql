...
-- validating its state 
SELECT system_user_id
,      system_user_name
FROM   system_user
WHERE  system_user_name = 'DBA';

-- ensure that iterative test cases all start at the same point, or common data state.
UPDATE system_user
...
-- verify the recent update to the table
SELECT system_user_id
,      system_user_name
FROM   system_user
WHERE  system_user_name LIKE 'DBA%';

-- create the initial procedure during iterative testing.
BEGIN
...
END insert_contact;
/ -- end contact insert procedure

DESC insert_contact

-- run the procedure in order to insert the value required
BEGIN
    insert_contact
    ( 'Charles'
    , 'Francis'
...
CREATE OR REPLACE PROCEDURE insert_contact
    ...
    -- add the precompiled instruction to make it an autonomous transaction
